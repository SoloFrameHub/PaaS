...
vi.mock('@/lib/ai/fetch-helpers', () => ({
    ...
    fetchLinkedinSnippet: vi.fn().mockResolvedValue({ headline: '', about: '', experience: '' }),
}));
...
describe('API Route: /api/onboarding/analyze', () => {
    ...
    it('should handle "analyzing" state concurrency', async () => {
        ...
        (profileService.getOrCreateProfile as any).mockResolvedValue(analyzingProfile);
    ...
    it('should run assessment flow when idle', async () => {
        ...
        expect(response.status).toBe(200);
        expect(response.status).toBe(200);
        // Expect at least one call with "analyzing" status, or just check the last state/sequence
