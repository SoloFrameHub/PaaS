      ...
      plugins: [{
        ...
        afterUpdate(c, args, options) {
          ...
          items?.forEach((item) => {
            ...
            button.onclick = () => {
              if (item.datasetIndex != null) c.setDatasetVisibility(item.datasetIndex, !c.isDatasetVisible(item.datasetIndex))
              c.update()
            }
            // Color box
            ...
            const dsIndex = item.datasetIndex ?? 0
            const theValue: number = (c.data.datasets[dsIndex]?.data as number[] ?? []).reduce((a: number, b: number) => a + b, 0)
