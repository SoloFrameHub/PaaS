 ...
 */
export async function createMockProfile(page: Page, profile: WellnessProfile) {
 ...
 */
export async function signInWithCompletedOnboarding(page: Page, userType: 'completed' | 'withProgress' | 'expanded' = 'completed') {
    ...
    // If still on onboarding, try one more time by navigating to dashboard
    if (currentUrl.includes('/onboarding')) {
        console.log('Stuck on onboarding, attempting manual navigation to dashboard...');
        await page.goto('/dashboard', { waitUntil: 'networkidle', timeout: 30000 });
        currentUrl = page.url();
    }

    if (currentUrl.includes('/onboarding')) {
        throw new Error(`Sign in failed - stuck on onboarding despite profile having onboardingCompleted: true. URL: ${currentUrl}`);
    }
 ...
 */
export async function signUpWithProfile(page: Page, profileOverrides: Partial<WellnessProfile> = {}) {
...
export async function signIn(page: Page, email = 'test@example.com', password = 'password123') {
    ...
    // In mock-auth mode the form POSTs to /api/auth/session; in real mode it POSTs
    // to /api/auth/signin. Accept either POST returning 2xx as a success signal.
    const [response] = await Promise.all([
        page.waitForResponse(
            resp => {
                const url = resp.url();
                const isAuth = url.includes('/api/auth/session') || url.includes('/api/auth/signin');
                const isPost = resp.request().method() === 'POST';
                if (isAuth && isPost) console.log(`Debug: Seen POST to ${url} with status ${resp.status()}`);
                return isAuth && isPost && resp.status() < 400;
            },
            { timeout: 15000 }
        ).catch((err) => {
            console.warn(`Sign in: Wait for auth response failed: ${err.message}`);
            return null;
        }),
...
/**
 * Sign in as a provider (mock auth only) by setting role='provider' in the session cookie.
 * Simulates a verified provider without needing a real DB entry.
 */
export async function signInAsProvider(
  page: Page,
  options: { uid?: string; email?: string } = {},
) {
  const uid   = options.uid   ?? 'mock-provider-001';
  const email = options.email ?? 'provider@example.com';

  await page.goto('/', { waitUntil: 'domcontentloaded' });

  const res = await page.request.post('/api/auth/session', {
    data: { idToken: 'mock-token', uid, email, role: 'provider' },
  });
  if (!res.ok()) throw new Error(`signInAsProvider: session POST failed ${res.status()}`);

  // Navigate somewhere so the cookie is definitely sent on subsequent requests
