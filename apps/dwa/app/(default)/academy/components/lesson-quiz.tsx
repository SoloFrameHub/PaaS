...
export default function LessonQuiz({ sectionId, courseId, lessonId }: LessonQuizProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (data.results?.passed) {
                window.dispatchEvent(new CustomEvent('exercise-completed', { detail: { type: 'quiz' } }));
            }
        } catch (err: any) {
    ...
    if (loading) {
        return (
            <section className="mt-12 p-4 sm:p-8 rounded-3xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-500/20">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-amber-500/20 flex items-center justify-center animate-pulse">
                        <span>📝</span>
                    </div>
                    <div>
                        <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100">Loading Quiz...</h3>
                    </div>
                </div>
            </section>
        );
    ...
    if (results) {
        return (
            <section data-testid="quiz-results" className="mt-12 p-4 sm:p-8 rounded-3xl bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/20 border border-amber-200 dark:border-amber-500/20">
                <div className="flex items-center gap-3 mb-6">
                    <div className="w-10 h-10 rounded-xl bg-amber-500 flex items-center justify-center text-white">
                        {results.passed ? '🎉' : '📚'}
                    </div>
                    <div>
                        <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100">Quiz Results</h3>
                        <p className="text-sm text-gray-500">
                            Score: <span className={`font-bold ${results.passed ? 'text-green-600' : 'text-amber-600'}`}>{results.score}%</span>
                            {results.passed ? ' - Passed!' : ' - Review and try again'}
                        </p>
                    </div>
                </div>
                    ...
                    {results.questionResults.map((qr, idx) => (
                        <div key={qr.id} className={`p-4 rounded-2xl border ${qr.isCorrect || qr.meetsMinLength
                            ...
                            <div className="flex justify-between items-start gap-2 mb-2">
                                ...
                                <span className={`shrink-0 text-xs font-black uppercase tracking-widest px-2 py-1 rounded-md ${qr.isCorrect || qr.meetsMinLength
                                    ? 'bg-green-500 text-white'
                                    : 'bg-red-500 text-white'
                                    }`}>
                                    {qr.isCorrect || qr.meetsMinLength ? 'CORRECT' : 'INCORRECT'}
                                </span>
                            </div>
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                                Your answer: <span className="font-medium underline decoration-2 underline-offset-2">{qr.userAnswer}</span>
                            </p>
                            ...
                            {qr.aiFeedback && (
                                <div className="mt-3 p-3 bg-primary-50 dark:bg-primary-900/20 rounded-xl border border-primary-100 dark:border-primary-800/30">
                                    <p className="text-xs font-bold text-primary-600 dark:text-primary-400 uppercase tracking-widest mb-1">AI Coaching</p>
    ...
    // Quiz form view
    return (
        <section data-testid="quiz" className="mt-12 p-4 sm:p-8 rounded-3xl bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/20 border border-amber-200 dark:border-amber-500/20">
            <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 rounded-xl bg-amber-500 flex items-center justify-center text-white">
                    📝
                </div>
                <div>
                    <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100">Test What You Learned</h3>
                    <p className="text-sm text-gray-500">Answer the questions below to check your understanding</p>
                </div>
            </div>

            <div className="space-y-8">
                {quiz.questions.map((q, idx) => (
                    <div key={q.id} data-testid="quiz-question" className="p-4 sm:p-6 rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700">
                        ...
                        {q.type === 'reflection' && (
                            <div>
                                <textarea
                                    ...
                                    placeholder={`Write your reflection here (minimum ${q.minLength || 50} characters)...`}
                                    value={answers[q.id] || ''}
                                    onChange={(e) => handleAnswerChange(q.id, e.target.value)}
                                />
                                <p className="text-xs text-gray-400 mt-1">
                                    {(answers[q.id]?.length || 0)} / {q.minLength || 50} characters minimum
                                </p>
                            </div>
                        )}
                    </div>
