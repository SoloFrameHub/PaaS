...
export const POST = withProviderAuth(async (req, { userId: providerId }) => {
  ...
  await withTenantApp(ctx, async (tx) =>
    tx.insert(providerInvite).values({ code, providerId, expiresAt }),
  );
...
export const PUT = withAuth(async (req, { userId: patientId }) => {
  ...
  const ctx = await requireTenantContext(req, { userId: patientId });
  ...
  const result = await withTenantApp(ctx, async (tx) => {
    ...
    return { status: 'ok' as const, providerId };
  });
