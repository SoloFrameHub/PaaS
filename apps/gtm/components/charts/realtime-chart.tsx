...
export default function RealtimeChart({
  ...
  useEffect(() => {
    if (chartValue.current && chartDeviation.current) {
      ...
      // B-047: textContent, not innerHTML — values are numeric today, but the
      // sink is indistinguishable from one that ships untrusted HTML, so
      // pinning to textContent removes the future-XSS vector.
