    ...
    <Menu as="div" className="relative inline-flex">
      {({ open }) => (
        <>
          ...
          >
            <span className="sr-only">{isEs ? '¿Necesitas ayuda?' : 'Need help?'}</span>
            <svg
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <div className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase pt-1.5 pb-2 px-3">{isEs ? '¿Necesitas ayuda?' : 'Need help?'}</div>
              ...
              <MenuItem as="li">
                {({ active }) => (
                  <Link className={`font-medium text-sm flex items-center py-1 px-3 ${active ? 'text-primary-600 dark:text-primary-400' : 'text-primary-500'}`} href="#0">
                    ...
                    <span>{isEs ? 'Documentación' : 'Documentation'}</span>
                  </Link>
                )}
              </MenuItem>
              <MenuItem as="li">
                {({ active }) => (
                  <Link className={`font-medium text-sm flex items-center py-1 px-3 ${active ? 'text-primary-600 dark:text-primary-400' : 'text-primary-500'}`} href="#0">
                    ...
                    <span>{isEs ? 'Soporte' : 'Support Site'}</span>
                  </Link>
                )}
              </MenuItem>
              <MenuItem as="li">
