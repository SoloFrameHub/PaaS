interface DocsTagProps {
  color: string;
  children: React.ReactNode;
}

export default function DocsTag({ color, children }: DocsTagProps) {
  ...
  return (
    <div
      className={`text-sm inline-flex font-medium rounded-sm text-center px-1 ${tagColor(color)}`}
    >
      {children}
    </div>
  );
