...
> = {
  ...
  help: { light: "fill-sky-400", dark: "fill-sky-600", text: "text-sky-600" },
};

export default function DocsTopicTitle({
  name,
  segment,
}: {
  name: string;
  ...
  const colors = topicColors[segment] || topicColors["getting-started"];
  ...
  return (
    <>
      ...
      <span className={`text-xl font-semibold ${colors.text} ml-4`}>
        {name}
      </span>
    </>
  );
