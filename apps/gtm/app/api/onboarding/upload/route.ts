...
export const POST = withAuth(async (request: NextRequest, { userId }) => {
  ...
  if (!expectedMimeTypes || !mimeTypeMatches) {
    logger.warn("Blocked suspicious file upload", {
      fileName: file.name,
      fileType: file.type,
      fileExtension,
    });
    throw new ValidationError(
      "Invalid file type or mismatched extension. Allowed: PDF, TXT, MD, JSON, DOC, DOCX",
    );
  }
  ...
  if (
    ...
    // DOC/DOCX: cannot extract text without a library — store raw with a warning
    content = `[DOC/DOCX file uploaded: ${file.name}. Text extraction not available — please upload as PDF or TXT for full analysis.]`;
    logger.warn("DOC/DOCX uploaded without text extraction", {
      fileName: file.name,
    });
  ...
  // Vectorize document in the background (don't block the upload response)
  if (content && !content.startsWith("[")) {
    vectorizeDocument(userId, docId, content, {
      fileName: file.name,
      documentType: "other",
    })
      .then(({ chunkCount }) => {
        logger.info("Document vectorized", { docId, chunkCount, userId });
      })
      .catch((err) => {
        logger.error("Document vectorization failed", { docId, error: err });
      });
  }
...
export const DELETE = withAuth(async (request: NextRequest, { userId }) => {
  ...
  await profileService.removeDocument(userId, documentId);
  deleteDocumentEmbeddings(userId, documentId).catch(() => {});
