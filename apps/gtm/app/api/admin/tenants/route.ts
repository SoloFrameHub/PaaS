// Admin endpoint for tenant onboarding. Gated by `checkAdminSecret` (Bearer
// $ADMIN_API_SECRET, timing-safe). Idempotent by slug — re-POSTing with the
// same slug returns the existing tenant id.
//
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await withSystemAdmin(async (tx) => {
      ...
      // future per-tenant GTM bootstrapping (e.g. starter pods) hangs off
      // this point.
