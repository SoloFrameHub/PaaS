'use client'

import { useState } from 'react'
import { useLocale } from 'next-intl'

export default function NotificationsPanel() {
  const locale = useLocale()
  const isEs = locale === 'es'

  const [comments, setComments] = useState<boolean>(true)
  const [messages, setMessages] = useState<boolean>(true)
  const [mentions, setMentions] = useState<boolean>(false)

  return (
    <div className="grow">

      {/* Panel body */}
      <div className="p-6 space-y-6">
        <h2 className="text-2xl text-gray-800 dark:text-gray-100 font-bold mb-5">
          {isEs ? 'Mis notificaciones' : 'My Notifications'}
        </h2>

        {/* General */}
        <section>
          <h3 className="text-xl leading-snug text-gray-800 dark:text-gray-100 font-bold mb-1">
            {isEs ? 'General' : 'General'}
          </h3>
          <ul>
            <li className="flex justify-between items-center py-3 border-b border-gray-200 dark:border-gray-700/60">
              {/* Left */}
              <div>
                <div className="text-gray-800 dark:text-gray-100 font-semibold">
                  {isEs ? 'Comentarios y respuestas' : 'Comments and replies'}
                </div>
                <div className="text-sm">
                  {isEs
                    ? 'Recibe una notificación cuando alguien comenta o responde a tu contenido.'
                    : 'Get notified when someone comments on or replies to your content.'}
                </div>
              </div>
              {/* Right */}
              <div className="flex items-center ml-4">
                <div className="text-sm text-gray-400 dark:text-gray-500 italic mr-2">
                  {comments ? (isEs ? 'Activo' : 'On') : (isEs ? 'Inactivo' : 'Off')}
                </div>
                <div className="form-switch">
                  <input type="checkbox" id="comments" className="sr-only" checked={comments} onChange={() => setComments(!comments)} />
                  <label htmlFor="comments">
                    <span className="bg-white shadow-sm" aria-hidden="true"></span>
                    <span className="sr-only">Enable smart sync</span>
                  </label>
                </div>
              </div>
            </li>
            <li className="flex justify-between items-center py-3 border-b border-gray-200 dark:border-gray-700/60">
              {/* Left */}
              <div>
                <div className="text-gray-800 dark:text-gray-100 font-semibold">
                  {isEs ? 'Mensajes' : 'Messages'}
                </div>
                <div className="text-sm">
                  {isEs
                    ? 'Recibe una notificación cuando alguien te envía un mensaje directo.'
                    : 'Get notified when someone sends you a direct message.'}
                </div>
              </div>
              {/* Right */}
              <div className="flex items-center ml-4">
                <div className="text-sm text-gray-400 dark:text-gray-500 italic mr-2">
                  {messages ? (isEs ? 'Activo' : 'On') : (isEs ? 'Inactivo' : 'Off')}
                </div>
                <div className="form-switch">
                  <input type="checkbox" id="messages" className="sr-only" checked={messages} onChange={() => setMessages(!messages)} />
                  <label htmlFor="messages">
                    <span className="bg-white shadow-sm" aria-hidden="true"></span>
                    <span className="sr-only">Enable smart sync</span>
                  </label>
                </div>
              </div>
            </li>
            <li className="flex justify-between items-center py-3 border-b border-gray-200 dark:border-gray-700/60">
              {/* Left */}
              <div>
                <div className="text-gray-800 dark:text-gray-100 font-semibold">
                  {isEs ? 'Menciones' : 'Mentions'}
                </div>
                <div className="text-sm">
                  {isEs
                    ? 'Recibe una notificación cuando alguien te menciona en una publicación o comentario.'
                    : 'Get notified when someone mentions you in a post or comment.'}
                </div>
              </div>
              {/* Right */}
              <div className="flex items-center ml-4">
                <div className="text-sm text-gray-400 dark:text-gray-500 italic mr-2">
                  {mentions ? (isEs ? 'Activo' : 'On') : (isEs ? 'Inactivo' : 'Off')}
                </div>
                <div className="form-switch">
                  <input type="checkbox" id="mentions" className="sr-only" checked={mentions} onChange={() => setMentions(!mentions)} />
                  <label htmlFor="mentions">
                    <span className="bg-white shadow-sm" aria-hidden="true"></span>
                    <span className="sr-only">Enable smart sync</span>
                  </label>
                </div>
              </div>
            </li>
          </ul>
        </section>

        {/* Shares */}
        <section>
          <h3 className="text-xl leading-snug text-gray-800 dark:text-gray-100 font-bold mb-1">
            {isEs ? 'Compartidos' : 'Shares'}
          </h3>
          <ul>
            <li className="flex justify-between items-center py-3 border-b border-gray-200 dark:border-gray-700/60">
              {/* Left */}
              <div>
                <div className="text-gray-800 dark:text-gray-100 font-semibold">
                  {isEs ? 'Compartidos de mi contenido' : 'Shares of my content'}
                </div>
                <div className="text-sm">
                  {isEs
                    ? 'Recibe una notificación cuando alguien comparte tu contenido.'
                    : 'Get notified when someone shares your content.'}
                </div>
              </div>
              {/* Right */}
              <div className="flex items-center ml-4">
                <button className="btn-sm border-gray-200 dark:border-gray-700/60 hover:border-gray-300 dark:hover:border-gray-600 shadow-sm">
                  {isEs ? 'Gestionar' : 'Manage'}
                </button>
              </div>
            </li>
            <li className="flex justify-between items-center py-3 border-b border-gray-200 dark:border-gray-700/60">
              {/* Left */}
              <div>
                <div className="text-gray-800 dark:text-gray-100 font-semibold">
                  {isEs ? 'Invitaciones al equipo' : 'Team invites'}
                </div>
                <div className="text-sm">
                  {isEs
                    ? 'Recibe una notificación cuando te invitan a unirte a un equipo.'
                    : 'Get notified when you are invited to join a team.'}
                </div>
              </div>
              {/* Right */}
              <div className="flex items-center ml-4">
                <button className="btn-sm border-gray-200 dark:border-gray-700/60 hover:border-gray-300 dark:hover:border-gray-600 shadow-sm">
                  {isEs ? 'Gestionar' : 'Manage'}
                </button>
              </div>
            </li>
            <li className="flex justify-between items-center py-3 border-b border-gray-200 dark:border-gray-700/60">
              {/* Left */}
              <div>
                <div className="text-gray-800 dark:text-gray-100 font-semibold">
                  {isEs ? 'Conexión inteligente' : 'Smart connection'}
                </div>
                <div className="text-sm">
                  {isEs
                    ? 'Sincronización automática y activa de tus datos entre herramientas conectadas.'
                    : 'Automatically and actively syncs your data across connected tools.'}
                </div>
              </div>
              {/* Right */}
              <div className="flex items-center ml-4">
                <div className="text-sm text-gray-400 dark:text-gray-500 italic mr-2 hidden md:block">
                  {isEs ? 'Activo' : 'Active'}
                </div>
                <button className="btn-sm border-gray-200 dark:border-gray-700/60 hover:border-gray-300 dark:hover:border-gray-600 shadow-sm text-red-500">
                  {isEs ? 'Desactivar' : 'Disable'}
                </button>
              </div>
            </li>
          </ul>
        </section>
      </div>

      {/* Panel footer */}
      <footer>
        <div className="flex flex-col px-6 py-5 border-t border-gray-200 dark:border-gray-700/60">
          <div className="flex self-end">
            <button className="btn dark:bg-gray-800 border-gray-200 dark:border-gray-700/60 hover:border-gray-300 dark:hover:border-gray-600 text-gray-800 dark:text-gray-300">
              {isEs ? 'Cancelar' : 'Cancel'}
            </button>
            <button className="btn bg-gray-900 text-gray-100 hover:bg-gray-800 dark:bg-gray-100 dark:text-gray-800 dark:hover:bg-white ml-3">
              {isEs ? 'Guardar cambios' : 'Save Changes'}
            </button>
          </div>
        </div>
      </footer>

    </div>
  )
}
