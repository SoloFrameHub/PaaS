'use client';

import { useLocale } from 'next-intl';

export default function BillingPanel() {
  const locale = useLocale();
  const isEs = locale === 'es';

  return (
    <div className="grow">

      {/* Panel body */}
      <div className="p-6 space-y-6">
        <div>
          <h2 className="text-2xl text-gray-800 dark:text-gray-100 font-bold mb-4">
            {isEs ? 'Facturación y facturas' : 'Billing & Invoices'}
          </h2>
          <div className="text-sm">
            {isEs
              ? <>El Plan Básico de este espacio está configurado en <strong className="font-medium">$34</strong> al mes y se renovará el <strong className="font-medium">9 de julio de 2024</strong>.</>
              : <>This workspace&apos;s Basic Plan is set to <strong className="font-medium">$34</strong> per month and will renew on <strong className="font-medium">July 9, 2024</strong>.</>}
          </div>
        </div>

        {/* Billing Information */}
        <section>
          <h3 className="text-xl leading-snug text-gray-800 dark:text-gray-100 font-bold mb-1">
            {isEs ? 'Información de facturación' : 'Billing Information'}
          </h3>
          <ul>
            <li className="md:flex md:justify-between md:items-center py-3 border-b border-gray-200 dark:border-gray-700/60">
              <div className="text-sm text-gray-800 dark:text-gray-100 font-medium">
                {isEs ? 'Método de pago' : 'Payment Method'}
              </div>
              <div className="text-sm text-gray-600 dark:text-gray-400 ml-4">
                <span className="mr-3">Mastercard ending 9282</span>
                <a className="font-medium text-primary-500 hover:text-primary-600 dark:hover:text-primary-400" href="#0">
                  {isEs ? 'Editar' : 'Edit'}
                </a>
              </div>
            </li>
            <li className="md:flex md:justify-between md:items-center py-3 border-b border-gray-200 dark:border-gray-700/60">
              <div className="text-sm text-gray-800 dark:text-gray-100 font-medium">
                {isEs ? 'Intervalo de facturación' : 'Billing Interval'}
              </div>
              <div className="text-sm text-gray-600 dark:text-gray-400 ml-4">
                <span className="mr-3">{isEs ? 'Anual' : 'Annually'}</span>
                <a className="font-medium text-primary-500 hover:text-primary-600 dark:hover:text-primary-400" href="#0">
                  {isEs ? 'Editar' : 'Edit'}
                </a>
              </div>
            </li>
            <li className="md:flex md:justify-between md:items-center py-3 border-b border-gray-200 dark:border-gray-700/60">
              <div className="text-sm text-gray-800 dark:text-gray-100 font-medium">
                {isEs ? 'Número de IVA/GST' : 'VAT/GST Number'}
              </div>
              <div className="text-sm text-gray-600 dark:text-gray-400 ml-4">
                <span className="mr-3">UK849700927</span>
                <a className="font-medium text-primary-500 hover:text-primary-600 dark:hover:text-primary-400" href="#0">
                  {isEs ? 'Editar' : 'Edit'}
                </a>
              </div>
            </li>
            <li className="md:flex md:justify-between md:items-center py-3 border-b border-gray-200 dark:border-gray-700/60">
              <div className="text-sm text-gray-800 dark:text-gray-100 font-medium">
                {isEs ? 'Tu dirección' : 'Your Address'}
              </div>
              <div className="text-sm text-gray-600 dark:text-gray-400 ml-4">
                <span className="mr-3">34 Savoy Street, London, UK, 24E8X</span>
                <a className="font-medium text-primary-500 hover:text-primary-600 dark:hover:text-primary-400" href="#0">
                  {isEs ? 'Editar' : 'Edit'}
                </a>
              </div>
            </li>
            <li className="md:flex md:justify-between md:items-center py-3 border-b border-gray-200 dark:border-gray-700/60">
              <div className="text-sm text-gray-800 dark:text-gray-100 font-medium">
                {isEs ? 'Dirección de facturación' : 'Billing Address'}
              </div>
              <div className="text-sm text-gray-600 dark:text-gray-400 ml-4">
                <span className="mr-3">hello@cruip.com</span>
                <a className="font-medium text-primary-500 hover:text-primary-600 dark:hover:text-primary-400" href="#0">
                  {isEs ? 'Editar' : 'Edit'}
                </a>
              </div>
            </li>
          </ul>
        </section>

        {/* Invoices */}
        <section>
          <h3 className="text-xl leading-snug text-gray-800 dark:text-gray-100 font-bold mb-1">
            {isEs ? 'Facturas' : 'Invoices'}
          </h3>
          <table className="table-auto w-full dark:text-gray-400">
            <thead className="text-xs uppercase text-gray-400 dark:text-gray-500">
              <tr className="flex flex-wrap md:table-row md:flex-no-wrap">
                <th className="w-full block md:w-auto md:table-cell py-2">
                  <div className="font-semibold text-left">{isEs ? 'Año' : 'Year'}</div>
                </th>
                <th className="w-full hidden md:w-auto md:table-cell py-2">
                  <div className="font-semibold text-left">{isEs ? 'Plan' : 'Plan'}</div>
                </th>
                <th className="w-full hidden md:w-auto md:table-cell py-2">
                  <div className="font-semibold text-left">{isEs ? 'Monto' : 'Amount'}</div>
                </th>
                <th className="w-full hidden md:w-auto md:table-cell py-2">
                  <div className="font-semibold text-right"></div>
                </th>
              </tr>
            </thead>
            <tbody className="text-sm">
              <tr className="flex flex-wrap md:table-row md:flex-no-wrap border-b border-gray-200 dark:border-gray-700/60 py-2 md:py-0">
                <td className="w-full block md:w-auto md:table-cell py-0.5 md:py-2">
                  <div className="text-left font-medium text-gray-800 dark:text-gray-100">2024</div>
                </td>
                <td className="w-full block md:w-auto md:table-cell py-0.5 md:py-2">
                  <div className="text-left">{isEs ? 'Plan Básico - Anual' : 'Basic Plan - Annually'}</div>
                </td>
                <td className="w-full block md:w-auto md:table-cell py-0.5 md:py-2">
                  <div className="text-left font-medium">$349.00</div>
                </td>
                <td className="w-full block md:w-auto md:table-cell py-0.5 md:py-2">
                  <div className="text-right flex items-center md:justify-end">
                    <a className="font-medium text-primary-500 hover:text-primary-600 dark:hover:text-primary-400" href="#0">HTML</a>
                    <span className="block w-px h-4 bg-gray-200 dark:bg-gray-700 mx-2" aria-hidden="true"></span>
                    <a className="font-medium text-primary-500 hover:text-primary-600 dark:hover:text-primary-400" href="#0">PDF</a>
                  </div>
                </td>
              </tr>
              <tr className="flex flex-wrap md:table-row md:flex-no-wrap border-b border-gray-200 dark:border-gray-700/60 py-2 md:py-0">
                <td className="w-full block md:w-auto md:table-cell py-0.5 md:py-2">
                  <div className="text-left font-medium text-gray-800 dark:text-gray-100">2024</div>
                </td>
                <td className="w-full block md:w-auto md:table-cell py-0.5 md:py-2">
                  <div className="text-left">{isEs ? 'Plan Básico - Anual' : 'Basic Plan - Annually'}</div>
                </td>
                <td className="w-full block md:w-auto md:table-cell py-0.5 md:py-2">
                  <div className="text-left font-medium">$349.00</div>
                </td>
                <td className="w-full block md:w-auto md:table-cell py-0.5 md:py-2">
                  <div className="text-right flex items-center md:justify-end">
                    <a className="font-medium text-primary-500 hover:text-primary-600 dark:hover:text-primary-400" href="#0">HTML</a>
                    <span className="block w-px h-4 bg-gray-200 dark:bg-gray-700 mx-2" aria-hidden="true"></span>
                    <a className="font-medium text-primary-500 hover:text-primary-600 dark:hover:text-primary-400" href="#0">PDF</a>
                  </div>
                </td>
              </tr>
            </tbody>
          </table>
        </section>
      </div>

      {/* Panel footer */}
      <footer>
        <div className="flex flex-col px-6 py-5 border-t border-gray-200 dark:border-gray-700/60">
          <div className="flex self-end">
            <button className="btn dark:bg-gray-800 border-gray-200 dark:border-gray-700/60 hover:border-gray-300 dark:hover:border-gray-600 text-gray-800 dark:text-gray-300">
              {isEs ? 'Cancelar' : 'Cancel'}
            </button>
            <button className="btn bg-gray-900 text-gray-100 hover:bg-gray-800 dark:bg-gray-100 dark:text-gray-800 dark:hover:bg-white ml-3">
              {isEs ? 'Guardar cambios' : 'Save Changes'}
            </button>
          </div>
        </div>
      </footer>

    </div>
  );
}
