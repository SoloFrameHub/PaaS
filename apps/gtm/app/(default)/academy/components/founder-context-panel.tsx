        ...
        <div className={`fixed right-0 top-1/2 -translate-y-1/2 z-50 transition-transform duration-500 ${isOpen ? 'translate-x-0' : 'translate-x-[calc(100%-12px)]'}`}>
            <div className="flex items-center">
                ...
                <div className="w-72 bg-gray-900 border-l border-white/10 p-6 shadow-2xl min-h-[400px]">
                    <div className="mb-8">
                        <h3 className="text-sm font-black uppercase tracking-widest text-primary-500 mb-2">{isEs ? 'Mi perfil de fundador' : 'My Founder Profile'}</h3>
                    ...
                    <div className="mb-8">
                        <h3 className="text-sm font-black uppercase tracking-widest text-primary-500 mb-2">{isEs ? 'Contexto de mercado' : 'Market Context'}</h3>
                        <div className="space-y-3">
                            <div className="flex justify-between items-center text-xs">
                                <span className="text-gray-500">{isEs ? 'Industria:' : 'Industry:'}</span>
                                <span className="text-white font-bold">{industry?.display_name || (isEs ? 'Genérico' : 'Generic')}</span>
                            </div>
                            <div className="flex justify-between items-center text-xs">
                                <span className="text-gray-500">{isEs ? 'Rol clave:' : 'Key Role:'}</span>
                    ...
                    <div className="mb-8">
                        <h3 className="text-sm font-black uppercase tracking-widest text-primary-500 mb-2">{isEs ? 'Objeciones comunes' : 'Common Objections'}</h3>
                    ...
                    <div className="text-[10px] text-gray-500 italic mt-auto pt-8 border-t border-white/5">
                        {isEs ? 'Tus lecciones y roleplays están afinados a este contexto.' : 'Your lessons and roleplays are currently tuned to this 3D Matrix context.'}
                    </div>
                </div>
            </div>
        </div>
