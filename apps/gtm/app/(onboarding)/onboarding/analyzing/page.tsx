...
const ANALYSIS_STEPS = [
    { id: 1, label: 'Reading your questionnaire responses...', labelEs: 'Leyendo tus respuestas del cuestionario...', duration: 2500 },
    { id: 2, label: 'Crawling provided URLs & LinkedIn...', labelEs: 'Rastreando URLs y LinkedIn...', duration: 3000 },
    { id: 3, label: 'Indexing uploaded documents (RAG)...', labelEs: 'Indexando documentos subidos (RAG)...', duration: 3500 },
    { id: 4, label: 'Calibrating DISC & Founder Archetype...', labelEs: 'Calibrando DISC y Arquetipo de Fundador/a...', duration: 2000 },
    { id: 5, label: 'Synthesizing GTM Strategy...', labelEs: 'Sintetizando Estrategia GTM...', duration: 3000 },
]
...
export default function AnalyzingPage() {
    ...
    // Stable ref for data to avoid re-running effect when data changes
    ...
    useEffect(() => {
        ...
        // Always run fresh analysis when user reaches this page through the onboarding flow.
        // Clear any stale assessment from server hydration or previous attempts.
        updateData({ analysisComplete: false, assessment: null, assessmentScore: 0 });

        const runAnalysis = async () => {
            // Capture data snapshot at analysis start (avoid stale closure issues)
            ...
            // Perform real analysis — send only what the server needs
            try {
                const response = await fetch('/api/onboarding/analyze', {
                    ...
                    body: JSON.stringify({
                        onboardingData: {
                            ...
                            revenueGoal: snapshot.revenueGoal,
                            stage: snapshot.stage,
                            linkedinUrl: snapshot.linkedinUrl,
                            linkedinAbout: snapshot.linkedinAbout,
                            linkedinPermission: snapshot.linkedinPermission,
                            questionnaire: snapshot.questionnaire,
                            uploadedDocuments: snapshot.uploadedDocuments?.map(d => ({
                                id: d.id,
                                name: d.name,
                                content: d.content,
                            })),
                ...
                // 1. Handle offline / network errors
                        ...
                        <div className="flex-1 flex items-center justify-center px-4 py-8">
                            <div className="max-w-md mx-auto text-center">
                                ...
                                <h1 className="text-2xl text-gray-800 dark:text-gray-100 font-bold mb-2">
                                    {isEs ? 'Analizando Tu Negocio' : 'Analyzing Your Business'}
                                </h1>
                                <p className="text-gray-600 dark:text-gray-400 mb-8">
                                    {isEs
                                        ? (ANALYSIS_STEPS[currentStep]?.labelEs || 'Casi listo...')
                                ...
                                <p className="text-sm text-gray-500">
                                    {Math.round(progress)}% {isEs ? 'completado' : 'complete'}
                                </p>
                                ...
                                {progress > 40 && (
                                    <div className="mt-12 p-6 bg-primary-50 dark:bg-primary-900/10 rounded-2xl border border-primary-100 dark:border-primary-800/50 animate-in fade-in zoom-in duration-700">
                                        <h3 className="text-primary-900 dark:text-primary-200 font-semibold mb-2">{isEs ? 'Descubrimiento Profundo en Progreso' : 'Deep Discovery in Progress'}</h3>
                                        <p className="text-sm text-primary-700 dark:text-primary-400 mb-4">
                                            {isEs
                                                ? 'Estamos realizando un análisis profundo de tus documentos y LinkedIn. Esto puede tomar unos minutos.'
                                                : 'We\'re performing a deep RAG analysis of your documents and deep-crawling your LinkedIn. This can take a few minutes.'}
                                        </p>
                                        <div className="flex justify-center">
                                            ...
                                            >
                                                {isEs ? 'Continuar al Panel' : 'Continue to Dashboard'}
                                            </button>
                                        </div>
                                    </div>
                                )}
                                ...
                                {data.companyName && (
                                    <div className="mt-8 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg text-left">
                                        <p className="text-xs text-gray-500 uppercase tracking-wide mb-2">{isEs ? 'Analizando' : 'Analyzing'}</p>
