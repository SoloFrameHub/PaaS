...
export default function FormClient({ formDefinition }: FormClientProps) {
  ...
  const validateStep = (): boolean => {
    ...
    for (const field of step.fields) {
      ...
      if (val === undefined || val === null || val === "") {
        errors[field.id] = isEs ? `${field.label} es requerido` : `${field.label} is required`;
      } else if (
        Array.isArray(val) &&
        val.length === 0 &&
        field.type === "multi-select"
      ) {
        errors[field.id] = isEs ? `Por favor selecciona al menos una opción` : `Please select at least one option`;
      }
      ...
      if (field.type === "url" && val && typeof val === "string") {
        try {
          ...
          errors[field.id] = isEs ? "Por favor ingresa una URL válida" : "Please enter a valid URL";
        }
      }
      ...
      if (field.type === "email" && val && typeof val === "string") {
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val)) {
          errors[field.id] = isEs ? "Por favor ingresa un correo electrónico válido" : "Please enter a valid email address";
        }
      }
      ...
      if (
        ...
        errors[field.id] = isEs
          ? `Debe tener al menos ${field.validation.minLength} caracteres`
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (result.error?.fieldErrors) {
          ...
          for (let i = 0; i < steps.length; i++) {
            ...
            if (
              Object.keys(result.error.fieldErrors).some((k) =>
                stepFieldIds.includes(k),
              )
            ) {
              setCurrentStep(i);
              break;
        ...
        setGlobalError(
          result.error?.message || (isEs ? "Error al enviar. Por favor intenta de nuevo." : "Submission failed. Please try again."),
        );
        setLoading(false);
        return;
      }
      ...
      // Store score results for the results page (readiness-score and similar)
      if (result.score !== undefined && result.scoreBreakdown) {
        sessionStorage.setItem(
          `${formDefinition.slug}-result`,
          JSON.stringify({
            score: result.score,
      ...
      setGlobalError(
        isEs ? "Error de red. Por favor verifica tu conexión e intenta de nuevo." : "Network error. Please check your connection and try again.",
      );
      setLoading(false);
    }
  };
      ...
      <div className="max-w-lg w-full">
        ...
        >
          ...
          <div className="flex items-center justify-between mt-8">
            ...
            >
              {loading ? (isEs ? "Enviando..." : "Submitting...") : isLastStep ? (isEs ? "Enviar" : "Submit") : (isEs ? "Continuar" : "Continue")}
            </button>
          </div>
        </form>
      </div>
