            ...
            <div className="max-w-6xl mx-auto mb-32 relative z-10">
                <div className="text-center mb-16">
                    <h2 className="text-3xl md:text-5xl font-serif text-white mb-6">
                        {isEs ? 'El Protocolo de Ventas de 7 Pistas' : 'The 7-Track Sales Protocol'}
                    </h2>
                    <p className="text-zinc-400 max-w-2xl mx-auto text-lg">
                        {isEs
                            ? 'Hemos desglosado la adquisición de clientes en 7 pistas lógicas. Avanza desde los fundamentos hasta la escala impulsada por IA en una ruta estructurada y orientada a la ingeniería.'
                    ...
                    {ACADEMY_TRACKS.map((track) => (
                        <div key={track.id} className={`group gradient-border p-6 rounded-2xl bg-zinc-900/20 backdrop-blur-sm hover:bg-zinc-900/40 transition-all ${track.isLargest ? 'lg:col-span-2' : ''}`}>
                            <div className="flex items-start justify-between mb-4">
                                ...
                                {track.isNew && (
                                    <span className="px-2 py-0.5 rounded bg-sky-500/20 text-sky-400 text-[10px] font-bold tracking-widest uppercase">
                                        {isEs ? 'Nuevo' : 'New'}
                                    </span>
                                )}
                            </div>
                            ...
                            <div className="flex items-center justify-between mt-auto pt-4 border-t border-white/5">
                                <span className="text-xs text-zinc-500 uppercase tracking-wider">
                                    {track.totalCourses} {isEs ? 'Cursos' : 'Courses'}
                                </span>
