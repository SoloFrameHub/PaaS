/**
 * Voice: OpenAI TTS + Whisper via dedicated voice client.
 * Voice endpoints aren't available on OpenRouter, so this always uses direct OpenAI.
 */

import { getVoiceClient } from '@/lib/ai/client';
...
export const voiceService = {
  ...
  async synthesizeSpeech(text: string): Promise<Uint8Array | null> {
    try {
      ...
      const response = await getVoiceClient().audio.speech.create({
  ...
  async transcribeAudio(audioBuffer: Buffer): Promise<string | null> {
    try {
      ...
      const transcription = await getVoiceClient().audio.transcriptions.create({
        file: stream as any,
