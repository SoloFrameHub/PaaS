...
Self-assessment slider. Optionally persists value via localStorage when `persistKey` is provided.
...
<RangeSlider
  ...
  persistKey="course1-lesson1-specificity"
/>
```
...
| `persistKey` | string | none | localStorage key (`slider-{persistKey}`). Omit for no persistence. |

**Storage:** `slider-{persistKey}` = number value (e.g., `7`). Without `persistKey`, resets on reload.
...
|-------------|-----------|-------------|
| `slider-{persistKey}` | RangeSlider | `number` (slider value) |
