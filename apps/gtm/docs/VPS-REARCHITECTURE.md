...
Re-architecting the course platform to run on a **Hostinger KVM 8 VPS (32GB RAM)** via **Dokploy**, with **no reliance on Genkit, Gemini, or any Google products**. Stack is TypeScript-native and self-hostable.
...
| **Hosting** | Firebase App Hosting | **Dokploy** on VPS |

---

## Dokploy & VPS

- **Dokploy**: Panel at `http://46.202.88.248:3000`, API key in `.env.local` as `DEPLOY_API_KEY`.
- **Inspect existing VPS** (projects, applications, services):
  ```bash
  npx tsx scripts/dokploy-status.ts
  ```
- Dokploy API: RPC-style endpoints (e.g. `POST /api/project.all`, `POST /api/application.deploy`). Auth via `x-api-key` header.
...
| Layer | Choice | Rationale |
|-------|--------|-----------|
| **Monitoring** | **Uptime Kuma** (Docker) + Dokploy dashboard | HTTP/checks; Dokploy for deploy status |
| **Logs / APM** | **Dokploy logs** or **Grafana Loki** (optional) | Centralized logs on VPS |
...
5. **Deploy & ops**  
   - Deploy Next.js app via Dokploy (Dockerfile build).
...
Remove from `.env.local` and any Dokploy/App Hosting config:
...
- **VPS**: Single Hostinger KVM 8 (32GB) with Dokploy; Trigger.dev, Postgres, Redis, optional MinIO/Umami/Uptime Kuma on same box; analytics, monitoring, and backup as above.

Use `npx tsx scripts/dokploy-status.ts` to inspect the current Dokploy VPS.
